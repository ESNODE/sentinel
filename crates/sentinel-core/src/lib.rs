...
impl Agent {
    pub fn new(config: AgentConfig, drivers: Vec<Box<dyn drivers::Driver>>) -> anyhow::Result<Self> {
        ...
        // WASM Skills
        for skill_cfg in &config.wasm_skills {
            info!("Loading WASM skill: {} from {:?}", skill_cfg.name, skill_cfg.path);
            let wasm_bytes = std::fs::read(&skill_cfg.path)
                .with_context(|| format!("Failed to read WASM skill file: {:?}", skill_cfg.path))?;
            
            let config_json = serde_json::to_string(&skill_cfg.params).unwrap_or_default();
            
            match crate::collectors::wasm::WasmCollector::new(
                ...
                Ok(collector) => {
                    collectors.push(Box::new(collector));
                    info!("WASM skill {} loaded successfully", skill_cfg.name);
                }
                Err(err) => {
                    warn!("Failed to load WASM skill {}: {:?}", skill_cfg.name, err);
                }
            }
        }
    ...
    pub async fn run(self) -> anyhow::Result<()> {
        ...
        let orchestrator_state_clone = if let Some(orch_config) = &config.orchestrator {
             if orch_config.enabled {
                let devices = vec![];
                let external_config = sentinel_orchestrator::OrchestratorConfig {
                ...
                if let Some(orch_app_state) = &orch_state_clone_for_update {
                    if let Ok(mut orch) = orch_app_state.orchestrator.write() {
                        ...
                        for gpu in gpu_status.iter() {
                            ...
                            let device = sentinel_orchestrator::Device {
                                id: id.clone(),
                                kind: sentinel_orchestrator::DeviceKind::Gpu,
        ...
        let enforcement_task = tokio::spawn(async move {
            ...
            // Enforcer needs to be Send. sentinel_core::control::Enforcer holds Nvml which is Send.
