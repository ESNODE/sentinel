        ...
        for (idx, gpu) in latest.gpus.iter().enumerate() {
            if let Some(prev_gpu) = prev.gpus.get(idx) {
                ...
                if prev_util > 50.0 && curr_util < (prev_util - 20.0) {
                    ...
                    // Check 5: PCIe Degradation (Gen or Width drop)
                    if let (Some(curr_gen), Some(prev_gen)) = (gpu.identity.as_ref().and_then(|i| i.pci_gen), prev_gpu.identity.as_ref().and_then(|i| i.pci_gen)) {
