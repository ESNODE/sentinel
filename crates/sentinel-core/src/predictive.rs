...
impl FailureRiskPredictor {
    ...
    pub fn analyze(&mut self, snapshot: &StatusSnapshot) -> HashMap<String, RiskAssessment> {
        ...
        for gpu in &snapshot.gpus {
                ...
                // Factor: Imminent Thermal Runaway
                if let (Some(curr_temp), Some(prev_temp)) = (gpu.temperature_celsius, history.last_temp) {
                    let dt = curr_temp - prev_temp;
                    if dt > 0.5 && curr_temp > 75.0 {
                        score += 40.0;
                        p_fail += 0.2;
                        factors.push(format!("Rapid temperature rise detected ({:.1}C/scrape at {:.1}C)", dt, curr_temp));
                    }
                }
                history.last_temp = gpu.temperature_celsius;
