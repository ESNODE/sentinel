...
use sentinel_core::state::StatusSnapshot;
...
impl AgentClient {
    ...
    #[allow(dead_code)]
    pub fn fetch_orchestrator_metrics(&self) -> Result<sentinel_orchestrator::PubMetrics> {
        let (status, body) = self.http_get("/orchestrator/metrics")?;
        if status == 404 {
            return Ok(sentinel_orchestrator::PubMetrics {
                device_count: 0,
                pending_tasks: 0,
                devices: vec![],
            });
        }
        let metrics: sentinel_orchestrator::PubMetrics =
