...
use sentinel_core::drivers::{Driver, SensorType};
use tracing_subscriber::{fmt, EnvFilter};

#[derive(Parser, Debug)]
#[command(name = "esnode-sentinel", version, about = "GPU-aware host metrics exporter")]
struct Cli {
    /// Optional path to configuration file (TOML). Also read from `SENTINEL_CONFIG`.
...
enum ConfigCommand {
    Show,
    Set {
        /// Key-value pair (key=value) to persist into sentinel.toml.
        key_value: String,
    },
}
...
async fn main() -> Result<()> {
    ...
    let mut config = sentinel_core::config::load_config(Some(config_path.clone()))
    ...
    match cli.command.as_ref().unwrap_or(&Command::Daemon) {
        Command::Daemon => {
            init_tracing(&config);
            tracing::info!("Starting ESNODE Sentinel with config: {:?}", config);
...
fn cli_to_overrides(cli: &Cli) -> Result<ConfigOverrides> {
    let orchestrator = if cli.enable_orchestrator.unwrap_or(false) {
        Some(sentinel_core::config::OrchestratorConfig {
            enabled: true,
            ..Default::default()
        })
    } else {
...
fn instantiate_drivers(config: &AgentConfig) -> Result<Vec<Box<dyn Driver>>> {
    ...
    for driver_cfg in &config.drivers {
        match driver_cfg.protocol.as_str() {
            "modbus" => {
                ...
                if let Some(register_str) = driver_cfg.params.get("registers") {
                    for reg_def in register_str.split(',') {
                        ...
                        if parts.len() >= 4 {
                            ...
                            register_mappings.push(sentinel_skill_modbus::RegisterMapping {
                                address,
                                count,
                                sensor_type,
                                unit: "raw".to_string(),
                ...
                drivers.push(Box::new(sentinel_skill_modbus::ModbusDriver::new(
                    driver_cfg.id.clone(),
                    addr,
                    slave_id,
                    register_mappings,
            ...
            "dnp3" => {
                ...
                let dnp3_config = sentinel_skill_dnp3::Dnp3Config {
                    local_addr,
                    remote_addr,
                    integrity_interval_ms: 5000,
                };
                
                drivers.push(Box::new(sentinel_skill_dnp3::Dnp3Driver::new(
                    driver_cfg.id.clone(),
                    addr,
                    dnp3_config,
                )));
            ...
            "snmp" => {
                ...
                let snmp_config = sentinel_skill_snmp::SnmpConfig {
                    target: addr,
                    community,
                    oids,
                };
                
                drivers.push(Box::new(sentinel_skill_snmp::SnmpDriver::new(
                    driver_cfg.id.clone(),
                    snmp_config,
                )));
            }
            "mqtt" => {
                ...
                let client_id = driver_cfg.params.get("client_id")
                    .cloned()
                    .unwrap_or_else(|| format!("sentinel-{}", driver_cfg.id));
                ...
                if let Some(mappings_str) = driver_cfg.params.get("mappings") {
                    for mapping_def in mappings_str.split(',') {
                        ...
                        if parts.len() >= 4 {
                            ...
                            topic_mappings.push(sentinel_skill_mqtt::TopicMapping::new(
                                parts[0].to_string(),
                                sensor_type,
                                parts[2].to_string(),
                                parts[3].to_string(),
                ...
                let mqtt_config = sentinel_skill_mqtt::MqttConfig {
                    broker,
                    port,
                    client_id,
                    username,
                ...
                drivers.push(Box::new(sentinel_skill_mqtt::MqttDriver::new(
                    driver_cfg.id.clone(),
                    mqtt_config,
                )));
            }
...
fn command_status(client: &AgentClient, no_color: bool) -> Result<()> {
    ...
    out.push_str("SENTINEL status (basic profile)\n");
    writeln!(
        &mut out,
        "  Healthy: {}",
...
fn command_metrics(client: &AgentClient, profile: MetricsProfile) -> Result<()> {
    println!("SENTINEL metrics snapshot (profile: {profile:?})");
...
fn command_diagnostics(client: &AgentClient) -> Result<()> {
    println!("Running SENTINEL diagnostics...");
...
fn command_plan(client: &AgentClient, profile_path: &Path) -> Result<()> {
    ...
    let profile: sentinel_core::policy::EfficiencyProfile = serde_yaml::from_str(&contents)
    ...
    for plan in result.matched_policies {
        let symbol = match plan.status {
            sentinel_core::policy::PlanStatus::Satisfied => "✅",
            sentinel_core::policy::PlanStatus::Violated => "❌",
            sentinel_core::policy::PlanStatus::Skipped => "⏭️",
        };
...
fn command_apply(client: &AgentClient, profile_path: &Path, yes: bool) -> Result<()> {
    ...
    // We need to import the EfficiencyProfile struct. Since sentinel-core exposes it in policy
    // but agent-bin depends on sentinel-core, we can access it.
    let profile: sentinel_core::policy::EfficiencyProfile = serde_yaml::from_str(&contents)
        .with_context(|| "failed to parse efficiency profile YAML")?;
    ...
    let violations: Vec<_> = result.matched_policies.iter().filter(|p| {
        matches!(p.status, sentinel_core::policy::PlanStatus::Violated)
    }).collect();
    ...
    let enforcer = sentinel_core::control::Enforcer::new();
...
mod tests {
    ...
    fn cli_parses_status_command() {
        let cli = Cli::parse_from(["esnode-sentinel", "status"]);
        assert!(matches!(cli.command, Some(Command::Status)));
    }

    #[test]
    fn cli_parses_metrics_command_with_profile() {
        let cli = Cli::parse_from(["esnode-sentinel", "metrics", "full"]);
        match cli.command {
